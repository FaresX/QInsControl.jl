...
function edit(fdg::FormatDataGroup, id)
    ...
    for (i, fd) in enumerate(fdg.data)
        ...
        if CImGui.BeginPopupContextItem()
            CImGui.MenuItem(stcstr(MORESTYLE.Icons.CloseFile, " ", mlstr("Delete"))) && deleteat!(fdg.data, i)
            CImGui.EndPopup()
        end
        CImGui.PopID()
...
function edit(dft::DataFormatter, id)
    ...
    if @c CImGui.Begin(stcstr(MORESTYLE.Icons.DataFormatter, " ", mlstr("Data Formatter"), "##", id), &dft.p_open)
        ...
        for (i, fd) in enumerate(dft.data)
            ...
            edit(fd, stcstr(id, '-', i))
            if CImGui.BeginPopupContextItem()
                CImGui.MenuItem(stcstr(MORESTYLE.Icons.CloseFile, " ", mlstr("Delete"))) && deleteat!(dft.data, i)
                CImGui.EndPopup()
            end
            CImGui.Spacing()
            if CImGui.BeginDragDropSource(0)
